program testgridestimate
    ...
    print *, 'INIT KERNEL'
    call kernel%Initialize( binSizes )
    kernelSmoothing       = spread( smoothing, 1, histogram%nActiveBins )
    !kernelSmoothingScale  = kernelSmoothing(:,1)*kernelSmoothing(:,2)*kernelSmoothing(:,3)

    call kernel%GridEstimateMulti( histogram%counts, nBins,  histogram%nActiveBins, &
                        histogram%activeBinIds, kernelSmoothing, outputGridEstimate )
